...
const DEPARTMENT_TILES = [
    ...
    },
    {
        ...
        colSpan: 'col-span-6 md:col-span-3', // Half width on mobile (next to Clothing)
        rowSpan: 'row-span-2', // Tall on mobile too
        color: 'bg-orange-50',
    ...
    return (
        <section className="relative z-10 max-w-[1200px] mx-auto px-6 py-8">
            <motion.div
                ...
                // Mobile: Grid (2 cols) with auto-rows | Desktop: Grid (4 cols) via grid-cols-12 and spans.
                className="grid grid-cols-12 gap-4 pb-4 auto-rows-[200px] md:grid-rows-3 md:gap-[20px] md:h-[840px] md:pb-0"
            >
                {DEPARTMENT_TILES.map((tile, index) => (
                    <motion.div
                        key={tile.title}
                        variants={itemVariants}
                        className={`relative group rounded-3xl overflow-hidden cursor-pointer
                            ${tile.colSpan} ${tile.rowSpan} // Grid Spans
                            h-full w-full // Height: fills grid area (defined by auto-rows or grid-rows)
                            ${tile.color}
                        `}
                    >
                        <Link href={tile.href} className="block h-full w-full relative z-10">
                            ...
                            <div className="absolute inset-0 p-5 md:p-8 flex flex-col justify-end text-white">
                                <div className="flex justify-between items-end">
                                    ...
                                    <div className="w-8 h-8 md:w-12 md:h-12 rounded-full bg-white/20 backdrop-blur-md flex items-center justify-center transform group-hover:-rotate-45 transition-all duration-500 shadow-xl group-hover:bg-white group-hover:text-navy-900 border border-white/30">
                                        <ArrowUpRight size={16} strokeWidth={1.5} className="md:w-6 md:h-6" />
                                    </div>
                                </div>
                            </div>
                        </Link>
