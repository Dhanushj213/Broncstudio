...
export default function MiniTrustStrip() {
    ...
    return (
        <div className="border-t border-b border-gray-100 bg-gray-50/50 py-6 mt-16">
            <div className="container-premium flex flex-wrap justify-between items-center gap-4 px-6 md:px-0">
                {features.map((f, i) => (
                    <div key={i} className="flex items-center gap-3 w-[45%] md:w-auto">
                        <div className="bg-white p-2 rounded-full shadow-sm text-navy-900">
                            <f.icon size={20} strokeWidth={1.5} />
                        </div>
                        <span className="text-sm font-medium text-navy-800">{f.text}</span>
                    </div>
                ))}
            </div>
        </div>
